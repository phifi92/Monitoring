#!/bin/bash
touch /tmp/dependancy_jeemonitor_in_progress
echo 0 > /tmp/dependancy_jeemonitor_in_progress
...
if [ "$versionPHP" -eq 5 ]
	then
		...
		echo "*****************************************************************************************************"
		sudo apt-get clean
		echo 20 > /tmp/dependancy_jeemonitor_in_progress
		sudo apt-get update
		echo 80 > /tmp/dependancy_jeemonitor_in_progress
		sudo apt-get install -y php-phpseclib
		echo 100 > /tmp/dependancy_jeemonitor_in_progress
	...
	then
		...
		echo 80 > /tmp/dependancy_jeemonitor_in_progress
		sudo apt-get install -y php-phpseclib
		echo 90 > /tmp/dependancy_jeemonitor_in_progress
		echo 100 > /tmp/dependancy_jeemonitor_in_progress
		echo "*****************************************************************************************************"
		echo "*  Pour finaliser l'installation, redémarrer Jeedom / To complete the installation, restart Jeedom  *"
		echo "*****************************************************************************************************"
...
rm /tmp/dependancy_jeemonitor_in_progress
