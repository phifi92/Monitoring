...
Initialement, ce plugin a été créé pour monitorer seulement Jeedom Mini et Jeedom Center. Mais pour répondre à une forte demande, il a été rendu compatible avec un Jeedom installé sur une VM (Machine Virtuelle) ou un NAS Synology

==== Compatibilité :

A ma connaissance, voici une liste non exhautive des hardware/distribution compatible:

- Jeedom Mini, Mini+ (surement la Center)
- Raspberry Pi
- Cubieboard
- Banana Pi
...
- Distributions spécifiques : OpenELEC, RasPlex et OSMC
...
- *CPU* : nombre de cœur, la fréquence associée et la température (température seulement avec Jeedom Mini)
